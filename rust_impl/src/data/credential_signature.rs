...
pub const CREDENTIAL_RULES: &[CredentialRule] = &[
    ...
    CredentialRule {
        patterns: &[r#"(?i)\b(?:password|passwd|pwd|passphrase)\s*[=:]\s*["']?([^\s"',;]+)"#],
        severity: Severity::High,
    ...
    CredentialRule {
        patterns: &[r"(?i)\bauthorization\s*:\s*(?:bearer|basic)\s+([A-Za-z0-9._+/=\-]{8,})"],
        severity: Severity::High,
    ...
    CredentialRule {
        patterns: &[r"(?i)\b(?:password|passwd|passphrase)\b"],
    ...
    CredentialRule {
        patterns: &[r"(?i)\b(?:username|login|signin|logon|credentials?)\b"],
        severity: Severity::Low,
    ...
    CredentialRule {
        patterns: &[r"(?i)\b(?:session|cookie|csrf|xsrf|jwt|saml)\b"],
        severity: Severity::Low,
        category: "Session / Web Auth",
    ...
    CredentialRule {
        patterns: &[r"(?i)\b(?:otp|hotp|totp|mfa|2fa)\b"],
        severity: Severity::Low,
    ...
    CredentialRule {
        patterns: &[r"(?i)\b(?:keystore|truststore|certificate|x509|pkcs12)\b"],
        severity: Severity::Low,
